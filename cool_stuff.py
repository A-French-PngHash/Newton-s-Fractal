...
def points_finding_roots_images(iterations : int, nb_points_line : int, nb_points_column : int, polynomial : dict, graph_scale):
    """
    ...
    for iteration in range(iterations):
        ...
        for (index, value) in enumerate(points):
            cord_x = value[0]
            cord_y = value[1]
            points[index] = newton_method.apply_newtons_method_complex(polynomial, derivative, [cord_x, cord_y], 1)


def points_final_location_closeness_to_root(iterations : int, nb_points_line, nb_points_column, polynomial : dict, roots : list, graph_scale = ((-2, 2), (-2, 2)), file_name : str = None) -> (list, list):
    """
    ...
    for (index, value) in enumerate(initial_points):
        cord_x = value[0]
        cord_y = value[1]
        new_cord_x, new_cord_Y = newton_method.apply_newtons_method_complex(polynomial, derivative, [cord_x, cord_y], iterations=iterations)
        min_distance = -1
        root_index = 0
