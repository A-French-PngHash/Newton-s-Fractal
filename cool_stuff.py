...
def points_finding_roots_images(iterations : int, nb_points_line : int, nb_points_column : int):
    ...
    :return:
    """
    test_polynomial = Polynomial([1, -1, 1, 0, 0, 1])
    derivative = test_polynomial.deriv()
    ...
    points = [complex(graph_scale[0][0] + (x / nb_points_line)*x_length, graph_scale[1][0] + (y / nb_points_column)*y_height) for x in
    ...
    for iteration in range(iterations):
        ...
        for (index, value) in enumerate(points):
            points[index] = newton_method.apply_newtons_method_complex(test_polynomial, derivative, value, 1)
...
def points_final_location_closeness_to_root(iterations : int, nb_points_line, nb_points_column, graph_scale = ((-2, 2), (-2, 2)), file_name : str = None) -> (list, list):
    ...
    initial_points = [
        complex(graph_scale[0][0] + (x / nb_points_line) * x_length_unit, graph_scale[1][0] + (y / nb_points_column) * y_height_unit) for
        x in
        range(nb_points_line + 1) for y in range(nb_points_column + 1)]
    ...
    for (index, value) in enumerate(initial_points):
        new_complex = newton_method.apply_newtons_method_complex(test_polynomial, derivative, value, iterations=iterations)
        min_distance = -1
        root_index = 0
        for (rind, element) in enumerate(roots):
            ndist = operations.distance_between_points((new_complex.real, new_complex.imag), element)
